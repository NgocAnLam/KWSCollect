...
async function updateSentence(formData: FormData) {
  "use server";
  const session = await getServerSession(authOptions);
  ...
  if (!session?.accessToken || !id || !text || !keyword) return;
  ...
  if (!res.ok) {
    ...
    throw new Error(data.detail ?? data.error ?? "Cập nhật thất bại");
  }
...
async function deleteSentence(id: number) {
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.detail ?? data.error ?? "Xóa thất bại");
  }
