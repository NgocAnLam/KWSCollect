/**
 * Validate sentence recording trên frontend (giống bước 3 keyword):
 * - Có nói hay không (im lặng / VAD)
 * - Có nói nhỏ hay không (âm lượng tối thiểu)
 * - Độ dài tối thiểu 5s cho câu dài.
 */
...
export async function validateSentenceAudio(blob: Blob): Promise<ValidateResult> {
  if (!blob || blob.size === 0) {
  ...
  if (durationSec < MIN_DURATION_SEC) {
    return {
      accepted: false,
      reason: `Bản thu quá ngắn (${durationSec.toFixed(1)}s). Cần ≥ ${MIN_DURATION_SEC}s.`,
    };
  }

  if (rmsDb < SILENCE_RMS_DB || voicedRatio < VAD_VOICED_RATIO) {
    return { accepted: false, reason: "Audio im lặng, vui lòng đọc to và rõ hơn." };
  }
